...
function OwnerLoginContent() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const result = login(identifier, password, 'OWNER');
      ...
      <div className="w-full max-w-[460px] mx-auto">
        <div className="bg-surface-container-lowest border border-outline-variant rounded-3xl shadow-sm p-6 sm:p-8 relative overflow-hidden">
          ...
          <div className="mb-6 text-left">
            <h1 className="font-headline-lg text-headline-lg text-primary tracking-tight font-bold">Facility Owner Login</h1>
            <p className="font-body-sm text-body-sm text-on-surface-variant mt-1">Manage your facility, courts, and real-time bookings.</p>
          </div>
          ...
          <form className="space-y-4" onSubmit={handleSubmit}>
            <div>
              <label className="block font-label-md text-label-md text-on-surface mb-1.5 font-semibold" htmlFor="identifier">
                Owner Email or Mobile Phone
              </label>
              <div className="relative">
                <div className="absolute inset-y-0 left-0 pl-3.5 flex items-center pointer-events-none text-on-surface-variant">
                  <span className="material-symbols-outlined text-[18px]">alternate_email</span>
                </div>
                <input required value={identifier} onChange={e => setIdentifier(e.target.value)} className="w-full h-[44px] pl-10 pr-3.5 bg-surface-container-lowest border border-outline-variant rounded-xl font-body-md text-body-md text-on-surface placeholder:text-outline placeholder:font-normal focus:border-primary-container focus:ring-1 focus:ring-primary-container transition-colors outline-none" id="identifier" name="identifier" placeholder="owner@quickcourt.in" type="text" />
              </div>
            </div>
          ...
          <div className="mt-8 pt-5 border-t border-outline-variant/60 text-center">
            <p className="font-body-sm text-body-sm text-on-surface-variant">
              Not a partner yet? 
              <Link href="/owner/facilities/new" className="font-label-md text-label-md text-primary font-bold hover:underline ml-1 inline-block">
